...
class SubCategoryController extends Controller
{
    ...
    public function index(Request $request){
        ...
        return view('backend.categorys.subcategory.index',compact('all'));
    }
    ...
    **/
    public function add(){
        ...
        return view('backend.categorys.subcategory.add',compact('totalpost','latestPost','all'));
    }
    ...
    **/
    public function view($id,$slug){
       ...
       //dd($data);
        return view('backend.categorys.subcategory.view',compact('data'));
    }
    ...
    **/
    public function restore($id){
        $data = SubCategory::withTrashed()->where('id', $id)->first();
        $data->restore();
        // Delete Successfully 
        if($data){
    ...
    **/
    public function delete($id){
        $data = SubCategory::onlyTrashed()->where('id', $id)->first();
    ...
    **/
    public function public_status($id,$slug){
        $published = SubCategory::where('id',$id)->where('slug',$slug)->where('public_status',0)->update([
            'public_status'=>1,
        ]);
        // Delete Successfully 
        if($published){
    ...
    **/
    public function private_status($id,$slug){
        $private = SubCategory::where('id',$id)->where('slug',$slug)->where('public_status',1)->update([
            'public_status'=>0,
        ]);
        // Delete Successfully 
        if($private){
    ...
    **/
    public function recycle(){
        $all = SubCategory::onlyTrashed()->get();
        return view('backend.categorys.subcategory.recycle',compact('all'));
    }
   ...
   /**
    ...
    public function bulkAction(Request $request)
    {
        ...
        if ($action === 'heard_delete') {
            $categories = SubCategory::onlyTrashed()->whereIn('id', $ids)->get();
        ...
        if ($action === 'active') {
            ...
            if($categories){
                foreach($categories as $data){
                    ...
                    SubCategory::whereIn('id',$ids)->where('public_status',0)->update([
                        'public_status'=>1,
                    ]);
                }
            }
        ...
        if ($action === 'deactive') {
            ...
            if($categories){
                foreach($categories as $data){
                    SubCategory::whereIn('id',$ids)->where('public_status',1)->update([
                        'public_status'=>0,
                    ]);
                }
            }
    ...
    public function export_pdf(){
       ...
       // return view('backend.categorys.subcategory.export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.categorys.subcategory.export_pdf', compact('categories')); // get database record 
    ...
    public function export_single_pdf($id,$slug){
        $data = SubCategory::where('id',$id)->where('slug',$slug)->firstOrFail();
       // return view('backend.categorys.subcategory.export_pdf', compact('categories'));
        $pdf = Pdf::loadView('backend.categorys.subcategory.export_single_pdf', compact('data'));
    ...
    public function export_excel(){
        return Excel::download(new CategoryExport, 'subcategory.xlsx');
    }
    ...
    public function export_csv(){
        return Excel::download(new CategoryExport, 'subcategory.csv');
    }
    ...
    public function export_zip()
    {
        ...
        // Export PDF file
        $pdf = Pdf::loadView('backend.categorys.subcategory.export_pdf', ['categories' => SubCategory::all()]);
